...
float pressureValue_BMP_oled;
float Temperature_BMP_oled;
// Compass
// Mode Control (MODE)
...
void setup() 
{
  ...
  attachInterrupt(digitalPinToInterrupt(SOS_PIN), sos_isr, RISING);
...
void loop() {
  ...
  pressureValue_BMP_oled = bmp.readPressure();
  Temperature_BMP_oled = bmp.readTemperature();
    ...
    if (menu_option != last_menu_option) {
      switch (menu_option) {
          ...
          case 2:
              ...
              oled.clearDisplay();
              oled.setCursor(0,0);
              oled.print("Temp body: ");
              oled.print(TC);
              oled.print(" C");
              oled.setCursor(0,20);
              oled.print("Pres: ");
              oled.print(pressureValue_BMP_oled/100);
              oled.print("hPa");
              oled.setCursor(0,40);
              oled.print("Temp area: ");
              oled.print(Temperature_BMP_oled);
              oled.print(" C");
              oled.display();
              break;
            case 3:
...
void sos_isr(){
  sos_flag=1;
  uint8_t sos_flagg = sos_flag;
   client.publish("SOS",String(sos_flagg).c_str());
